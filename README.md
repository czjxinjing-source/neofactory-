...
当前项目没有引入 Electron 打包工程，桌面端主要通过本地启动脚本打开浏览器运行。
...
如果系统阻止 PowerShell 脚本运行，可以改用 `launcher` 目录里的 `.cmd` 文件，或在终端手动执行 `npm run dev -- --port 5173`。
...
构建完成后会生成 `dist/` 目录。`dist/` 是构建产物，备份源码或交给 ChatGPT / Codex 继续开发时通常不需要包含。
...
# neofactory-training
随便练练
>>>>>>> 23eeb9861c42752a0707587c10197d9cc15cd40b
